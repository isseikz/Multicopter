...
def main():
    ...
    # initial guess for gain matrix K
    K = np.array([
        [-0.001, 0.5, 3.0, -3.0, -1.0, 0.5, 3.0*0.99, -3.0*0.99, -0.99],
        [-0.001, 0.5, 3.0, 3.0, 1.0, 0.5, 3.0*0.99, 3.0*0.99, 0.99],
        [-0.001, 0.5, -3.0, -3.0, 1.0, 0.5, -3.0*0.99, -3.0*0.99, 0.99],
        [-0.001, 0.5, -3.0, 3.0, -1.0, 0.5, -3.0*0.99, 3.0*0.99, -0.99]
    ])
    K_p = np.zeros((4, 9))
    J = np.dot(ref_pos, ref_pos.T)
    J_p = -1
    alpha_dK = 0.01
    J_best = J
    K_best = K_p
    T_terminal = 0.0
    diff_best = [0.0, 0.0, 0.0]
    T_best = 0.0

    while J_best > 0.0: # 1 sec/ 100steps
        T_terminal += 0.01
        if time % 1000 == 0:
            print(time/100)
            # model.show_status()
        ...
        if (time % 200 == 0) | any( abs(angle) > np.pi/3 for angle in pred_state.get_euler_angle()):
            # update controller
            if time > 10:
                # 1. calc criterion


                diff = ref_pos - pred_state.get_position()
                # print(diff)
                J = np.dot(diff, diff.T) # - T_terminal
                if J_best > J:
                    print(J, diff, T_terminal)
                    J_best = J
                    K_best = K
                    diff_best = diff.copy()
                    T_best = T_terminal
                ...
                K = np.dot(3, np.random.rand(4,9)) - np.dot(1.5, np.ones((4, 9)))
                # print(K)
        ...
        position = pred_state.get_position()
        velocity = pred_state.get_velocity()
        quartanion = pred_state.get_quartanion()
        ...
        vel_alt = pred_state.get_velocity()[2]
        attitude = pred_state.get_euler_angle()
        attitude_deg = np.array(attitude) * 180 / math.pi
        ang_vel = pred_state.get_angular_velocity()
        integral += (-1.0-altitude)

        delta_x = np.array([
                integral,
                -(ref_pos[2] - altitude),
                (ref_roll - attitude_deg[0])/180.0,
                (ref_pitch - attitude_deg[1])/180.0,
                (ref_yaw - attitude_deg[2])/180.0,
                -(vel_alt),
                -ang_vel[0]/(2*math.pi),
                -ang_vel[1]/(2*math.pi),
                -ang_vel[2]/(2*math.pi)
            ])
        new_inputs = np.dot(K, delta_x)# 左後，左前，右後，右前
        ...
        # log_est_t.append(pred_state.get_time())#log_est_t.append(model.dynamics.get_time())
        ...
        sensor_acc = pred_state.get_acceleration()
        dnn_output = np.hstack((
            (sensor_acc
            - mf.convert_vector_inertial_to_body(
                model.gravity,
                pred_state.get_quartanion())) / 9.81
            ,
            pred_state.get_angular_velocity()))
